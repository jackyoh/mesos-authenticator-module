...
public:
      ...
      connection(nullptr)
  {
    ...
    char string[sizeof(credential.secret().data())];
    strcpy(string, credential.secret().data());
    ...
    LOG(INFO)<<"md5 digest:"<<mdString;

    size_t length = sizeof(mdString);
    ...
    memcpy(secret->data, mdString, length);
    secret->len = length;
  }
